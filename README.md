...
当前版本：**0.2.5**。Python >=3.9，无第三方运行时依赖，包含类型标记 `py.typed`。

版本变化见[变更日志](https://github.com/narutozb/pysvnlite/blob/main/CHANGELOG.md)；main 分支可能包含尚未发布的修改，正式发行包以对应标签为准。

批量目标文件与原生编码约束见 [API 参考](https://github.com/narutozb/pysvnlite/blob/main/docs/api-reference.md#批量目标)。
...
```bash
python -m pip install "pysvnlite==0.2.5"
svn --version --quiet
```
