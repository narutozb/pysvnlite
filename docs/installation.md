...
适用于 pysvnlite 0.2.1。要求 Python >=3.9、pip、PATH 中可用的 `svn`。运行时不依赖第三方 Python 包；`svnadmin` 仅用于本地集成测试。
...
```powershell
python -m venv .venv
.\.venv\Scripts\python.exe -m pip install "pysvnlite==0.2.1"
...
```bash
python3 -m venv .venv
.venv/bin/python -m pip install "pysvnlite==0.2.1"
...
```bash
python -m pip install --upgrade "pysvnlite==0.2.1"
```

0.2.1 不改变现有 API。若同时使用 svnpypi，检查其依赖范围仍允许本版本，并执行两者的实际导入与业务测试。
...
若仍需包注册 CLI，再安装 `svnpypi==0.2.1`。不要降级安装旧捆绑版到同一环境。旧项目若仅使用 SVN API，其依赖声明应改为 pysvnlite，而不是继续安装 svnpypi 获取底层库。
...
```bash
python -m pip download --only-binary=:all: --no-deps --dest wheelhouse "pysvnlite==0.2.1"
python -m pip install --no-index --find-links wheelhouse "pysvnlite==0.2.1"
```
