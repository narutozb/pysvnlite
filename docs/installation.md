...
```powershell
python -m venv .venv
.\.venv\Scripts\python.exe -m pip install "pysvnlite==0.2.3"
```
...
```bash
python3 -m venv .venv
.venv/bin/python -m pip install "pysvnlite==0.2.3"
```
...
```bash
python -m pip install --upgrade "pysvnlite==0.2.3"
```
...
```bash
python -m pip download --only-binary=:all: --no-deps --dest wheelhouse "pysvnlite==0.2.3"
```

目标环境安装：

```bash
python -m pip install --no-index --find-links wheelhouse "pysvnlite==0.2.3"
```
