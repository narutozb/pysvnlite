...
以下为回复结构，尖括号内容须替换为实际命令、脚本或证据；不能将占位内容直接交给报告者执行。

```text
状态：修复已推送到 <PR 链接>；未合并，未发布。
修复提交：<完整 SHA 及固定链接>；基线：<原报告版本/来源>。
验证环境：<操作系统、Python、SVN 客户端及必要工具>。
隔离安装：<创建新虚拟环境、安装固定 SHA、核对实际导入来源的完整命令>。
最小验收：<完整脚本或固定提交中的脚本链接及执行命令>。
预期：<具体输出/断言，原故障不再出现且相关兼容行为不变>。
实测：<实际环境、提交、命令、通过/失败/跳过>。
未验证：<原报告的宿主、认证或平台差异；无则写无>。
反馈：请附安装来源、环境及脱敏后的实际结果；无需修改生产环境。
```

以 [#15 的交付记录](https://github.com/narutozb/pysvnlite/issues/15#issuecomment-5797915497)为例：该记录验证的是 [PR #17](https://github.com/narutozb/pysvnlite/pull/17) 的固定提交 `0c57b5b14b06ae630593c975a33db99b0a76eea3`，不是 PyPI 的 `0.2.3`。记录内提供新虚拟环境安装命令、来源 SHA 断言和临时 SVN 仓库脚本；预期字面量 `@` 文件能返回正确状态，同时保留字符串空 peg 行为。此例仅说明该次交付，不代表 PR 当前合并或发布状态。

修复验证不能只提供全量测试数量或维护者本机路径；报告者应能从公开的固定入口取得同一份代码并独立判断结果。
